...
var tomSelectOp = document.getElementById('select-state')
if (tomSelectOp) {
  // Initialize Tom Select
  var select = new TomSelect('#select-state', {
    valueField: 'name',
    labelField: 'name',
    searchField: 'name',
  ...
  const element = document.querySelector('#select-state')
  let arr = []
  for (let i = 0; i < element.options.length; i++) {
    const label = element.options[i].text;
    const value = element.options[i].value;
    if (element.options[i].getAttribute('selected') == 'selected') {
      arr.push(label)
    }
